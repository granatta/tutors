...
francese_bp = Blueprint("francese", __name__)

PAROLA_FILE = "parola_cache_fr.json"
TEMA_HISTORY_FILE = "tema_history_fr.json"
DICTIONARY_API_BASE = "https://api.dictionaryapi.dev/api/v2/entries/fr"

SYSTEM_FRANCESE = """Ti chiami Chloé e sei un tutor di lingua francese per studenti di scuola secondaria di primo grado italiana (11-14 anni, livello QCER A1-A2). Se uno studente ti chiede come ti chiami, rispondi che ti chiami Chloé.

REGOLE FONDAMENTALI:
1. Spiega regole grammaticali e istruzioni in italiano, chiaro e adatto a 11-14 anni; usa il francese per esempi, frasi guida e piccoli dialoghi di pratica.
2. Mantieni il livello linguistico coerente con le Indicazioni Nazionali per la scuola secondaria di primo grado (QCER A1-A2): lessico quotidiano, presente indicativo, verbi ausiliari e modali di base, frasi semplici; introduci il genere dei sostantivi (le/la), l'accordo di articoli e aggettivi e i tempi verbali diversi dal presente solo con gradualità e mai in modo avanzato senza che siano stati introdotti dallo studente stesso.
3. Quando è utile mostrare il significato, la pronuncia o un uso reale di una parola o espressione francese, usa lo strumento cerca_parola per recuperare dati reali invece di inventarli.
4. Non affrontare mai argomenti fuori dal perimetro della materia (contenuti non adatti all'età, altre materie scolastiche, opinioni personali su temi delicati): riporta con gentilezza lo studente sulla lingua francese.
5. Correggi gli errori con gentilezza, mostrando sempre la forma corretta (incluso l'articolo/genere del sostantivo, es. le/la) e una breve spiegazione del perché.

STRUTTURA DELLE RISPOSTE:
- Spiega in modo chiaro e breve, poi fai fare pratica con una piccola domanda o un mini-esercizio.
- Usa **grassetto** per le parole o espressioni chiave in francese.
- Tieni le risposte brevi e coinvolgenti, mai un trattato."""

TEMI_PAROLA = [
    "la vie scolaire", "la nourriture et les boissons", "les animaux", "le sport", "la famille",
    "les loisirs", "la météo", "les voyages", "la technologie",
    "les émotions", "la ville", "la routine quotidienne",
]
...
TOOLS = [
    {
        "name": "cerca_parola",
        "description": (
            "Cerca una parola francese reale in un dizionario online (definizione, pronuncia, esempi). "
        ...
        "input_schema": {
            ...
            "properties": {
                "parola": {
                    "type": "string",
                    "description": "La parola francese da cercare, in forma base/infinito o al singolare (es. 'parler', non 'parle')"
                }
            },
            "required": ["parola"]
        }
...
def cerca_parola_dizionario(parola):
    """Cerca una parola francese reale su dictionaryapi.dev. Restituisce un dict
...
@francese_bp.route("/francese")
def pagina_francese():
    return render_template("tutor_francese.html")


@francese_bp.route("/parola-del-giorno-francese")
...
#        if cache.get("data") == oggi:
    ...
    prompt = (
        f"Sto creando materiale didattico di lingua francese per studenti italiani di scuola media "
        f"(11-13 anni, livello QCER A1-A2). Scegli UN sostantivo o una parola francese comune e utile legata al tema: \"{tema}\".\n\n"
        "Rispondi SOLO con un oggetto JSON valido, senza testo prima o dopo, con questa struttura esatta:\n\n"
        "{\n"
        '  "parola": "la parola francese scelta, con l\'articolo se è un sostantivo (es. \'le chien\')",\n'
        ...
        '  "frase_esempio_fr": "una frase di esempio in francese, semplice, livello A1-A2",\n'
...
@francese_bp.route("/chat-francese", methods=["POST"])
def chat_francese():
    data = request.json
